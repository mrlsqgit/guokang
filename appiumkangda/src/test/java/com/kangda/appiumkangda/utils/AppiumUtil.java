...
public class AppiumUtil {
	...
	public void swipe(int beginX, int beginY, int endX, int endY) { 
		...
		try {
			...
			logger.info("从(" + beginX + "," + beginY + ")滑动到(" + endX + "," + endY + ")");
		} catch (Exception e) {
			e.printStackTrace();
			logger.error("滑动失败");
		}
...
//	 }
	 ...
	 * @param during
	 * @param num
	 *            driver（就是把appiumdriver对象传进来） during（这里是填写毫秒数，这里的 毫秒数越小
	 *            滑动的速度越快~ 一般设定在500~1000，如果你想快速滑动 那就可以设置的更加小） num（是只滑动的次数，本人在做相册
	 *            翻页测试什么的 滑动 或者滑动到列表底部。就直接输入次数就行了）
	 */
